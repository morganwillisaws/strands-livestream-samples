...
def create_university_agent(actor_id: str = "default_user", session_id: str | None = None):
    """Create the University chatbot agent with AgentCore Memory."""
...
def invoke(payload, context):
    ...
    # The CLI wraps the entire JSON input as a string in the "prompt" field
    raw_prompt = payload.get("prompt", "Hello")

    # Try to parse it as JSON to extract structured fields
    try:
        parsed = json.loads(raw_prompt)
        prompt = parsed.get("prompt", raw_prompt)
        actor_id = parsed.get("actor_id", "demo_user")
        session_id = parsed.get("session_id", "persistent_demo_session")
    except (TypeError, json.JSONDecodeError):
        prompt = raw_prompt
        actor_id = payload.get("actor_id", "demo_user")
        session_id = payload.get("session_id", "persistent_demo_session")

    logger.info(f"invoke: actor_id={actor_id}, session_id={session_id}, prompt={prompt[:50]}")

    agent = create_university_agent(actor_id=actor_id, session_id=session_id)
    logger.info(f"agent.messages count after init = {len(agent.messages)}")
